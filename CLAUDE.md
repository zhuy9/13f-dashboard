...
- `13F-HR` and `13F-HR/A` are both fetched, and the amendment type decides what happens (Milestone 11). `RESTATEMENT` replaces the filing it corrects; `NEW HOLDINGS` adds only previously-confidential positions and must be **unioned** with the original — letting it replace the original drops the real portfolio. An amendment with no type on its cover page is skipped and printed, never guessed at. `resolve_amendments()` in `fetch.py` owns this; the fetch window counts report *periods*, not filings, so an amended quarter cannot push the oldest quarter out.
- Splits are never inferred. `ingest/corporate_actions.json` carries them as data with a source per entry, and `split_factor()` restates the prior quarter's shares before status is decided. An unexplained clean multiple sets `split_unverified` (flagged in the UI as `UNADJUSTED?`) and adjusts nothing. The flag keys on value as well as shares, because a split leaves a position's value alone while a purchase moves both.
- A **`13F-NT`** (notice) means the manager holds positions but reported none itself — another manager filed them. Its cover page lists that manager under "List of Other Managers Reporting for this Manager" (name + CIK). `fetch_filings` asks only for `13F-HR`, so an NT quarter would otherwise look exactly like a manager that stopped filing — `stale_manager_lines()` tells the two apart and says which in the run summary. Add the named CIK to `aliases13f`; do not swap the roster `cik`, which would orphan every `managers/`, `manager_quarters/` and `ownership_investors/` doc.
